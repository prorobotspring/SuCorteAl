<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
	...
	<xsl:variable name="textCargosAprox" select="' tiene un estimado de Bs.S '" />
	...
	<!-- DEFINICION DEL MENSAJE PARA CLIENTES CON PRORRATEO -->
	<xsl:variable name="mensajeClientesProrrateo" select="'Disculpe, el servicio de Corte 123 estará nuevamente disponible a partir de su próxima Fecha de Corte de Factura'" />
	<!-- DEFINICION DEL MENSAJE PARA CLIENTES CON PRODUCTOS INVALIDOS -->
	<xsl:variable name="mensajeClientesError" select="'Disculpe, el servicio de Corte 123 no está disponible en este momento'" />
	...
	<xsl:template match="mannindia:return">
		...
		<xsl:variable name="first" select="concat(concat(format-number( substring-before($newString, ','),'###.###.###','espaniol'),','),substring(substring-after($newString, ','),1,3))" />
		...
		<xsl:choose>
			...
			<xsl:when test="$SMS_FORMAT = 'Formato 1'">
				...
				<xsl:choose>
					<xsl:when test="$VOZ_D_D &gt; 9999998">
						<xsl:value-of select="$Ilimitado" />
					</xsl:when>
					<xsl:when test="$VOZ_D_D &lt; 9999999">
			...
			<xsl:when test="$SMS_FORMAT = 'Formato 3'">
				...
				<xsl:choose>
					<xsl:when test="$VOZ_412 &gt; 9999998">
						<xsl:value-of select="$Ilimitado" />
					</xsl:when>
					<xsl:when test="$VOZ_412 &lt; 9999999">
			...
			<xsl:when test="$SMS_FORMAT = 'Formato 10'">
		...
		<xsl:if test="ax27:errorCode = 3000">
			<xsl:value-of select="$mensajeClientesProrrateo" />
		</xsl:if>
		
		<xsl:if test="ax27:errorCode = 3001">
			<xsl:value-of select="$mensajeClientesError" />
		</xsl:if>
		
		<xsl:if test="ax27:errorCode = 3002">
			<xsl:value-of select="$mensajeClientesError" />
		</xsl:if>
